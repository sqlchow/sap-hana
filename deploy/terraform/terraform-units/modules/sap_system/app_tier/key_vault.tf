...
resource "azurerm_key_vault_secret" "app_auth_username" {
  count        = local.sid_local_credentials_exist && local.enable_deployment && length(local.sid_kv_user) > 0 ? 1 : 0
...
resource "azurerm_key_vault_secret" "app_auth_password" {
  count        = local.enable_auth_password && local.enable_deployment && local.sid_local_credentials_exist  && length(local.sid_kv_user) > 0 ? 1 : 0
